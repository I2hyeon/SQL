-- 뷰
-- 뷰는 제한적인 데이터를 쉽게 보기 위해서 미리 만들어놓은 가상 테이블(가짜) 입니다
-- 자주 사용되는 컬럼을 저장하면 관리가 용이합니다
-- 뷰는 물리적으로 데이터가 저장된 것은 아니고 원본 테이블을 기반으로 한 가사테이블이라고 생각하면 됩니다
-- 뷰를 만들기 위해서는 권한이 필요합니다 (HR) 권한을 가지고 있습니다

SELECT * FROM EMP_DETAILS_VIEW; -- 미리 만들어져 있는 뷰
SELECT * FROM USER_ROLE_PRIVS; -- 유저의 권한 확인
