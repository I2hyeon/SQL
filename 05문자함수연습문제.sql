...
SELECT CONCAT(FIRST_NAME, LAST_NAME) AS 이름, REPLACE(HIRE_DATE, '/', '') AS 입사일자 FROM EMPLOYEES ORDER BY 이름;
...
SELECT LOWER(RPAD(SUBSTR(FIRST_NAME, 1, 3), LENGTH(FIRST_NAME), '*')) AS NAME, LPAD(SUBSTR(SALARY, 1, 4), 10, '*') FROM EMPLOYEES WHERE JOB_ID IN ('IT_PROG');