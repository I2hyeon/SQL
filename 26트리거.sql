-- 트리거 - 트리거는 테이블에 부착한 형태로 AFTER, BEFORE 트리거 있습니다
-- AFTER DML 문장이 타겟테이블에 실행된 이후에 동작하는 트리거
...
DELETE TBL_TEST WHERE ID = 'AAA123'; --트리거 동작
--------------------------------------------------------------------------------
-- AFTER TRIGGER
-- :OLD = 참조전 값 (I : 입력전 자료 V :변경전 자료 D :삭제전 자료)
...
-- TBL_USER에 업데이트 OR 딜리트가 일어나면 기존데이터를 백업
CREATE OR REPLACE TRIGGER TBL_USER_BACKUP
...
CREATE OR REPLACE TRIGGER TBL_USER_MAKING_TAG
    BEFORE INSERT
    ON TBL_USER
    FOR EACH ROW
BEGIN
