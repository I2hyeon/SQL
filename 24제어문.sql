...
DECLARE
    POINT NUMBER := TRUNC( DBMS_RANDOM.VALUE(1, 101) ); -- 1 ~ 101 미만의 랜덤수
BEGIN
...
BEGIN
    ...
    LOOP
        FOR I IN 1..9
        LOOP
            DBMS_OUTPUT.PUT_LINE(CNT || ' X ' || I || ' = ' || CNT * I);
        END LOOP;
...
BEGIN
        ...
        LOOP
            ...
            EXIT WHEN X%NOTFOUND; --  X 커서의 더이상 읽을 값이 없으면 TRUE
...
BEGIN
    OPEN X;
        DBMS_OUTPUT.PUT_LINE('----------커서 시작----------');
    LOOP
        FETCH X INTO DEPARTMENT_ID, SALARY;
        EXIT WHEN X%NOTFOUND;
        
        DBMS_OUTPUT.PUT_LINE('부서 ID : ' || DEPARTMENT_ID); 
        DBMS_OUTPUT.PUT_LINE('급여합 : ' || SALARY);
...
DECLARE
    A NUMBER;
    B NUMBER;
    CURSOR X IS SELECT A, SUM(SALARY) AS B FROM 
                                        (SELECT TO_CHAR(HIRE_DATE, 'YYYY') 
                                        AS A, SALARY FROM EMPLOYEES) GROUP BY A;
BEGIN
