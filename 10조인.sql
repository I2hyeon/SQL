...
SELECT INFO.ID,
       INFO.TITLE,
       INFO.CONTENT,
       INFO.AUTH_ID, -- AUTH_ID는 양쪽에 다 있는 KEY, 테이블.컬럼명 기입해줘야 합니다
       AUTH.NAME
FROM INFO 
/*INNER*/ JOIN AUTH
...
-- 연결할 키가 같다면 UISING 구문을 사용할 수 있음
SELECT *
FROM INFO I
INNER JOIN AUTH A
USING (AUTH_ID);
...
--------------------------------------------------------------------------------
-- OUTER JOIN
-- LEFT OUTER JOIN (OUTER 생략가능) - 왼쪽 테이블이 기준이 돼서 왼쪽 테이블은 다 나온다
 SELECT * FROM INFO I LEFT OUTER JOIN AUTH A ON I.AUTH_ID = A.AUTH_ID;
 
 -- RIGHT OUTER JOIN - 오른쪽 테이블이 기준이 돼서 오른쪽 테이블은 다 나온다
...
-- CROSS JOIN (잘못된 조인의 형태로 실제로 쓸 일 없음)
...
--------------------------------------------------------------------------------
-- 오라클 조인 - 오라클에서만 사용 가능하며 조인할 테이블을 FROM절에 씀, 조인 조건을 WHERE절에 씀
...
-- 크로스조인은 잘못된 조인(조인 조건을 안 적었을때 나타남)
SELECT *
FROM INFO I,AUTH A;
